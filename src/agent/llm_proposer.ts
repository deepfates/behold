...
export function startLLMProposer(bot: Bot, specs: ToolSpec[], enqueue: (i: Intent) => void, opts: Options) {
  ...
  async function decideOnce() {
    try {
      ...
      const intent = await callLLM(specs, frame, opts);
      if (!intent) return;
      if (allow && !allow.has(intent.tool)) return;
      enqueue(intent);
    } catch (e: any) {
      log(`[llm] error: ${e?.message || String(e)}`);
    }
  }

  function start() {
    if (timer) return;
    timer = setInterval(decideOnce, tickMs);
  }
...
function getFrame(bot: any) {
  ...
  const lastChat = null; // could be wired similarly to REPL cache later
  return { position, health: bot?.health, food: bot?.food, time, isDay, dimension: dim, lastChat };
}

async function callLLM(specs: any[], frame: any, opts: Options): Promise<Intent | null> {
  const system = [
    'You control a Minecraft bot by calling exactly one tool per turn (or sending a short chat message).',
    'Prefer: observe (status/nearby/cursor) → move_to/look_at → act (dig/place/equip/eat).',

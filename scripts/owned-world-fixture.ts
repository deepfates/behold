...
export const OWNED_TARGET = Object.freeze({ x: 3, y: -60, z: 0, item: 'apple', count: 1 });

export async function prepareOwnedWorld(runIdValue: string, port: number, label = 'owned-world') {
  ...
  const world: WorldLabDefinition = {
    ...
    notes: [
      ...
      `One dropped ${OWNED_TARGET.item} affordance is prepared at ${OWNED_TARGET.x},${OWNED_TARGET.y},${OWNED_TARGET.z}.`,
    ],
  };
...
async function generatePreparedWorld(input: {
  ...
  for (const command of [
    ...
    `summon minecraft:item ${OWNED_TARGET.x} ${OWNED_TARGET.y} ${OWNED_TARGET.z} {Item:{id:"minecraft:${OWNED_TARGET.item}",count:${OWNED_TARGET.count}}}`,
  ]) {

...
looking, movement, communication, attack, and digging choices; Pine's resumed
dig produced a Minecraft-confirmed mud-bricks-to-air change. Resume reopened the
saved bodies and supplied Elm only its own first five turns and Pine only its
own first six. Successful local decisions had median upstream latency of about
9.7 seconds initially and 12.4 seconds after resume; both episodes saved and
stopped cleanly. Textile independently read the exact 563,992-byte frozen union
as 15 turns plus two roots with no unsupported or nonconforming results. The
additive `org.behold.inhabitant.v2` presentation pact and vendored Lync 0.4.2
make its sound, time, whisper, action, failure, and verified material consequence
readable without exposing coordinates or private controller/provider fields.
...
- `behold live` can own Place's ordinary `serve --control-jsonl` child from either the historical exact checkout (`--place-compiler`) or a read-only installed CLI. Installed mode requires `--place-compiler-bin`, `--place-compiler-version`, and `--place-compiler-distribution-sha256`; take the latter two values from that binary's `place-compiler version --json`. Behold repeats that preflight before serving and requires the unchanged `place-compiler-serve-control/v1` wire and schema-V3 capability. Both modes use the same lifecycle parser, exact release/world/server/runtime identities, and fail-closed shutdown.
...
- `BEHOLD_POLICY_PROFILE` — continuous-transcript candidate `resident-v3`, action-only ordinary `resident-v2` (default), coached legacy `resident-v1`, strategy-minimal evaluation `neutral-benchmark-v1`, or narrated legacy treatment `legible-resident-v1`
...
- `resident-v3` is the acceptance candidate for an uninterrupted private model conversation. It deterministically projects canonical Lync as chronological experience, exact retained assistant response, and Minecraft outcome messages, then appends one fresh complete current view. It refuses event gaps and context overflow rather than folding or truncating a middle. OpenRouter requires context-bound private route v5; LM Studio binds its loaded context and uses a conservative wire-size admission. See [Resident transcript](docs/RESIDENT_TRANSCRIPT.md). This path is implemented, deterministically tested, and exercised by a no-world request over Rowan's complete retained Oxford life, but it is not yet the default or ordinary live-proven treatment.

...
runtime defect discovered in the prior Qwen release was first corrected with
distinct entity-bound model instances. The subsequent v3 session boundary keeps
the isolation that matters without duplicating immutable weights: residents on
the same exact model/config share one LM Studio instance, with parallel slots
capped by broker concurrency, while prompts, broker credentials, accounting,
processes, and Lync state remain resident-owned. Neither correction alters
choices or rescues Qwen's repeated-camera behavior.
...
candidate earned admission, there was no new live/resume/POV/Lync-history episode.
...
Commit `6cb902e` initially made the model-instance identity a digest of the exact
policy and stable entity identity. The v3 session boundary supersedes that
weight-level isolation: managed startup groups identical model/config policies,
loads one exact custom instance, and reserves no more parallel slots than the
broker can concurrently admit. The session record retains each
entity-to-instance binding. Each
controller still receives only its admitted instance identity; each physical
request includes the complete resident-owned prompt; the authenticated broker
keeps credentials and quota accounts separate; release unloads and verifies the
shared owned instance once. Artifact preflight remains deduplicated because two
residents using one immutable artifact do not require hashing it twice.

This is implemented and fixture-exercised, not live-exercised. A same-model
two-resident test proves one load, one shared exact ID, `parallel = 2`, two
resident bindings, and one unload; the same fixture proves a concurrency-one
broker can cap that runtime at `parallel = 1`. The broker still validates exact
request and returned runtime identity, while its existing per-resident
credential boundary owns admission and accounting. The managed runner test
proves that the admitted ID crosses release, controller environment, and cleanup.

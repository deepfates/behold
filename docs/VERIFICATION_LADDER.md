...
| 5. Learnable episode | A bounded episode can reference exact life and world-history ranges, become a dataset example, and support optimization without becoming private memory or changing the environment. | Lync episode references, immutable request/program identities, train/held-out separation, and a held-out real rollout.          | Evaluator-owned Lync episodes now preserve exact branch-stable life ranges; runtime/program bindings are omitted until call and lifecycle evidence can authenticate them, and assessment, world-lineage, dataset, and held-out proofs remain red. |

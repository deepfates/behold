...
| -------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ | ----------------------------------------------------------------------------------------------------------------------------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| 1. Honest turn       | A bounded observation leads to one admitted decision; observation, proposal, authorization, execution, world fact, and outside check remain distinguishable where they apply.        | Contract tests, one live decision binding, one live world-action binding, a structurally checked action graph, and a world-native material witness where claimed. | Live v4 evidence and clean-process reassessment pass for one uncoached Ax action with no invented fact. A separate scripted native-body proof passes for an air-to-dirt change confirmed by callback-time `blockUpdate`, resident terminal delivery, post-quiescence fresh-body read, Lync life, and recomputed material fact. The Minecraft claim is green; cross-world conformance belongs to rung 4.                                         |
...
| 3. Replaceable mind  | Direct, Ax, scripted, local, and future minds can receive the same request and return the same decision contract without gaining body authority.                                     | Exact request identity, adapter provenance, matched proposal replays, and one matched real rollout.                                                               | Direct and content-addressed Ax passed exact immutable-request differentials, a six-call repeated sample with adapter and resource distributions, and real causal rollouts from sibling copies of one checkpoint. One Ax-started private life then continued through two more epochs under direct, preserving its Lync and prior consequences. The Minecraft claim is green; more adapters and cross-world continuation remain useful coverage. |
...
| 5. Learnable episode | A bounded episode can reference exact life and world-history ranges, become a dataset example, and support optimization without becoming private memory or changing the environment. | Lync episode references, immutable request/program identities, train/held-out separation, and a held-out real rollout.                                            | Three neutral live turns now bind exact requests, mind/program identities, world-history starting and ending digests, runtime epochs, Lync ranges, and evaluator episodes; every range survives later life appends and clean reassessment. Dataset indexing, lineage-safe split integrity, and held-out improvement remain red.                                                                                                                 |
...
| 7. Forked histories  | Worlds can checkpoint, fork, continue, and be re-entered without confusing a place artifact, runtime epoch, world history, life, or evaluation episode.                              | Immutable checkpoint lineage, isolated writable children, branch-local clocks, crossing receipts, and honest identity behavior.                                   | Minecraft now seals a stopped source, verifies immutable checkpoint and Lync lineage, runs isolated writable siblings, and re-enters one diverged history for three epochs while one private life continues across a mind change. Checkpoint/fork/re-entry is green. Crossing receipts, authorized life crossing/copy, and longer branch-clock evidence remain red.                                                                             |
...
5. **Name the witness strength.** Fresh terminal delivery proves that the body
   received the action lifecycle. A material-effect claim additionally needs a
   fresh body or native world evidence. Never call two projections of one
   controller event independent world truth.
...
1. Turn the foundation ladder into versioned, recomputable claim records with
   explicit dependencies, verifier commands, evidence identities, and budgets.
2. Prove one honest turn and continuation through a second real world adapter.
   Only then extract a shared package from the semantics both integrations
   actually need.
3. Define checkpoint, history branch, runtime epoch, life, crossing, and copy
   identity before claiming matched rollouts or held-out splits.
4. Run direct and content-addressed Ax minds over isolated writable children of
   one checkpoint, then optimize only against lineage-safe training episodes.
5. Apply the resulting conformance and lineage gates to population work.
